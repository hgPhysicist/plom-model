        ...
        .on('end', function(){
          ...
          that.load('metadata', 'N', options.root, function(err, pop_size){
            ...
            best.forEach(function(mybest, i){
              ...
              thetas.push(_addUngroupedStates(mytheta, that.par_sv, states[i], that.getPopSize_n(pop_size, states[i], n)));
...
Theta.prototype.plugHat = function(options, callback){
  ...
  fs.exists(pathHat, function (exists) {
    ...
    parse.obj_n(fs.createReadStream(pathHat), {key: 'time', n:n}, function(err, hat_n){
      ...
      that.load('metadata', 'N', options.root, function(err, pop_size){
        if(err) return callback(err);        
        that._plugHat(hat_n, that.getPopSize_n(pop_size, hat_n, n), options);
        callback(null);
      });
    });
...
//TODO use event emitter for log
