...
Theta.prototype.validate = function(){
  ...
  //validate parameters
  missing = _.difference(this.par_sv.concat(this.par_proc, this.par_obs, this.volatility, this.whiteNoiseSd), Object.keys(this.theta.parameter));
  ...
  var aliens = _.difference(Object.keys(this.theta.parameter), this.par_sv.concat(this.par_proc, this.par_obs, this.volatility, this.whiteNoiseSd));
  if( aliens.length ){
    throw new Error(util.format('in theta.json, parameter [%s] have to be deleted', aliens.join(',')));
  }

  this.par_sv.concat(this.par_proc, this.volatility, this.whiteNoiseSd).forEach(function(p){
  ...
  this.par_sv.concat(this.par_proc, this.par_obs, this.volatility, this.whiteNoiseSd).forEach(function(p, i, allPar){
    if(p in that.theta.parameter){
      var myp = that.theta.parameter[p];
  ...
  this.context.population.forEach(function(p){
    ...
    that.process.state.forEach(function(x){
      if(x.id in that.theta.parameter){        
        ...
        sumSv += parg.guess.value;
        if(parg.sd_transf.value > 0.0){
          sumSvFree += parg.guess.value;
        }
      }
    });
    ...
    if(sumSv >= 1.0){
      ...
      that.process.state.forEach(function(x){
        if(x.id in that.theta.parameter){        
          ...
          if(parg.sd_transf.value > 0.0){            
            var guessr = (parg.guess.value/sumSvFree)*norm;                        
            var transf = that.theta.parameter[x.id]['transformation'];

            if( ( (guessr < parg.min.value) || (guessr > parg.max.value) ) && ( (transf === 'logit_ab') || (transf === 'scale_pow_10_bounded') || (parg.prior.value === 'uniform') ) ){
              throw new Error(util.format('%s %s cannot be normalized (%d -> %d) without breaking the transformation or the prior', x.id, p.id, parg.guess.value, guessr));
            } else {
  ...
  for (var par in this.theta.parameter){
    for (var group in this.theta.parameter[par]['group']){
      ...
      if(parg.sd_transf.value > 0.0){
        ...
        if( (("prior" in parg) && (parg.prior.value === "uniform")) || ( (transf === 'logit_ab' || (transf === 'scale_pow_10_bounded')) ) ){
        ...
        } else if( ((transf === 'log') || (transf === 'scale_pow_10_neg')) && (x <= 0.0) ){
...
Theta.prototype._sanitize = function(par, group, options){
  ...
  if( ((transf === 'log') || (transf === 'scale_pow_10_neg')) && (x < 0.0) ){
