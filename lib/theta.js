    ...
    .on('end', function(){
      ...
      csv()
        ...
        .on('end', function(){
          ...
          that.load('N', function(err, pop_size){
            if (err) return callback(err);
            ...
            best.forEach(function(mybest, i){
              ...
              thetas.push(that._addUngroupedStates(mytheta, states[i], that._getPopSize_n(pop_size, states[i], n)));
...
Theta.prototype.plugBest = function(options, callback){
  ...
  pathBest = path.resolve(options.root || '.', pathBest);

  fs.exists(pathBest, function (exists) {
    if(! exists){ 
      return callback(new Error(util.format('%s could not be found.', pathBest)));
    }

    parse.obj_n(fs.createReadStream(pathBest), {n: options.index_best}, function(err, best_n){
      if(err) return callback(err);
      reduce.merge(that.theta, best_n, options);
      callback(null);      
    });
...
Theta.prototype.plugHat = function(options, callback){
  ...
  fs.exists(pathHat, function (exists) {
    ...
    parse.obj_n(fs.createReadStream(pathHat), {key: 'time', n:n}, function(err, hat_n){
      ...
      that.load('N', function(err, pop_size){        
        if(err) return callback(err);        
        that._plugHat(hat_n, that._getPopSize_n(pop_size, hat_n, n), options);
        callback(null);
      });
    });
 ...
 */
Theta.prototype._addUngroupedStates = function(theta, hat_n, pop_size_n){
...
Theta.prototype._plugHat = function(hat_n, pop_size_n, options){
  ...
  if(options.ungroup){
    that._addUngroupedStates(that.theta, hat_n, pop_size_n);
  } else {
    ...
    this.par_sv.forEach(function(par) {
      ...
      for(var p in p2g){
        par_object['group'][p2g[p]]['guess']['value'] += hat_n[par + ':' + p] / pop_size_n[p];
      }
...
Theta.prototype._rescale = function(data, hat, options){
  ...
  for(var n=1; n< data.length; n++){ //skip header
    for(var ts=1; ts< data[0].length; ts++){ //skip date      
      if(data[n][ts] !== null){
        mean_data[ts-1] += data[n][ts]; 
        n_data[ts-1]++;
      }
    }
  }
  ...
  var offset_hat = 2+this.par_sv.length*this.context.population.length*3;
  for(var n=1; n< hat.length; n++){ // skip header
    for(var ts=0; ts< data[0].length-1; ts++){ //skip date
  ...
  for(var ts=0; ts< data[0].length-1; ts++){
