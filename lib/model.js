...
Model.prototype.validate = function(){
  ...
  var sv=this.par_sv
    ...
    , obs=this.par_obs
    , meta = this.par_meta
    , states = this.process.state.map(function(x) {return x.id}) //also contains remainder as opposed to sv  
  ...
  //check process model
  var allowedPrate = this.op.concat(states, proc, meta, funcsPLOM, exprsPLOM); //terms allowed in rates of the process model
  ...
  this.process.model.forEach(function(s, i){
    ...
    if('tag' in s){ 
      if(!Array.isArray(s.tag)){
        throw new Error(util.format('in process.json, model reaction: "%d" tag has to be a list', i));
      } else if (! s.tag.length) {
        throw new Error(util.format('in process.json, reaction: "%d" tag can not be empty', i));
      } else if ( (_.difference(s.tag, ['transmission'])).length ) {
        throw new Error(util.format('in process.json, reaction: "%d" invalid tag (%s)', _.difference(s.tag, ['transmission'])));
      }
    }
  ...
  if(this.process.white_noise){
    ...
    this.process.white_noise.forEach(function(s,j){
  ...
  if(this.process.diffusion){
	...
	throw new Error(util.format('in process.json, white noise %d, reactions must have a %s property', (_.difference(['parameter', 'drift','volatility'], Object.keys(s))).join(', ')));            
      ...
      if ( proc.indexOf(obj.parameter) === -1 && obs.indexOf(obj.parameter) === -1){
	...
	throw new Error(util.format('We are sorry, but non-zero drifts for time-varying parameters are not yet supported.',s.parameter));
      ...
      if ( proc.indexOf(obj.volatility) === -1 ){
  ...
  var allowedOrate = this.op.concat(states, obs, meta, funcsPLOM, exprsPLOM); //terms allowed in rates of the observtion model
...
Model.prototype._getPopSize_n = function(hat_n){
  var that = this;
