...
function Model(context, process, link, options){
  ...
  this.volatility = ('diffusion' in this.process) ? this.process.diffusion.map(function(x) {return x.volatility}) : [];
  this.whiteNoiseSd = ('white_noise' in this.process) ? this.process.white_noise.map(function(x) {return x.sd}) : [];

  //remove par_fixed
  this.par_proc = _.difference(this.par_proc, this.par_meta);
  this.volatility = _.difference(this.volatility, this.par_meta);
  this.whiteNoiseSd = _.difference(this.whiteNoiseSd, this.par_meta);
