    ...
    if(('source' in m) && (typeof m.source !== 'string') ){ //data have been parsed, we check      
      ...
      if(mandatory){
        ...
        for(var s in m.source){
          ...
          if(datesNonNull.length < 2){
            throw new Error(util.format('in context.json, metadata "%s" (%s), need at least to non null values encompassing [%s - %s]', m.id, s, dateMin, dateMax));
          }
