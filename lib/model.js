...
Model.prototype.validate = function(){
  ...
  var states = this.process.state.map(function(x) {return x.id}) //also contains remainder as opposed to sv  
    ...
    , funcsPLOM = ['terms_forcing','heaviside','ramp','sin','cos','tan','correct_rate'];
  ...
  this.link.observed.forEach(function(s, i){
	...
	if(states.indexOf(r.from)>-1 && that.par_sv.indexOf(r.from)==-1 && r.from != 'U' ){
	  throw new Error(util.format('Incidence relative to reactions leaving from the remainder state %s cannot be observed.',r.from));
	};
...
Model.prototype._checkMetadata = function() {
  ...
  this.context.metadata.forEach(function(m){
    ...
    if(('source' in m) && (typeof m.source !== 'string') ){ //data have been parsed, we check      
      ...
      if( (pproc.indexOf(m.id) !== -1)  || (m.id === 'N') ){
        mandatory = popId;                
      ...
      if(mandatory){
        ...
        for(var s in m.source){
          ...
          var datesNonNull = m.source[s].value.filter(function(x){return x[1] !== null});
          if(datesNonNull.length < 2 & !(that.pop_size_eq_sum_sv & m.id==="N")){
            throw new Error(util.format('in context.json, metadata "%s" (%s), need at least two non null values encompassing [%s - %s]', m.id, s, dateMin, dateMax));
          }
                 
          myt0 = new Date(datesNonNull[0][0]); //first date non null          
          mytend = new Date(datesNonNull[datesNonNull.length-1][0]); //last date non null
          
          if(myt0 > dateMin){
            throw new Error(util.format('in context.json, metadata "%s" (%s), the first non null point (%s) has to be before data.t0 (%s)', m.id, s,  myt0, dateMin));
          }
          if(mytend < dateMax  & !(that.pop_size_eq_sum_sv & m.id==="N")){
            throw new Error(util.format('in context.json, metadata "%s" (%s), the last non null point (%s) has to be after the last data point (%s)', m.id, s, mytend, dateMax));
          }
        }
      }
    } else {
