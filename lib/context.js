...
function getFrequency(data){
  ...
  var d1 = new Date(data[1][0]) 
    , d2 = new Date(data[2][0]);
 
  //difference in days
  var delta = Math.round((d2.getTime() - d1.getTime()) / (1000 * 60 * 60 * 24));  
  var freq = {
    1: 'D', 
    7: 'W', 
    14: 'B',
    28: 'M', 30: 'M', 31: 'M', //TODO re-interpolate data if frequency === 'M'
    365: 'Y'
  };

  if (! delta in freq){
    throw new Error('could not get frequency');
  }
...
Context.prototype.parseDataSync = function(){
  ...
  if(data.length){
    this.context.frequency = getFrequency(data.filter(function(x){return x.id === 'data'})[0]['source']);   
  } 
...
Context.prototype.parseData = function(callback){
  ...
  async.each(data, function(d, cb){string2data(d, 3, cb);}, function(err){
    ...
    try{
      that.context.frequency = getFrequency(data.filter(function(x){return x.id === 'data'})[0]['source']);
    } catch(e){
      return callback(e);
    }
